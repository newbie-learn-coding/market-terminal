  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center gap-2 border-b border-white/[0.08]">
        ...
        <div>
          <CardTitle>{t('narrativesTitle')}</CardTitle>
          <CardDescription>{t('narrativesDesc')}</CardDescription>
        </div>
      </CardHeader>
        ...
        {isEmpty ? (
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2 text-[11px]">
              <Badge variant="orange" className="mono">{t('clusters')} {narrativeStats.count}</Badge>
              {narrativeStats.rising ? <Badge variant="teal" className="mono">{t('rising')} {narrativeStats.rising}</Badge> : null}
              {narrativeStats.steady ? <Badge variant="orange" className="mono">{t('steady')} {narrativeStats.steady}</Badge> : null}
              {narrativeStats.fading ? <Badge className="mono">{t('fading')} {narrativeStats.fading}</Badge> : null}
            </div>
